...
- **v4 1단계 시안 초안 완료 · 검토 대기 (2026-09-14).** 소개 3장 · 홈 구성 1장 · 구성값을 반영한 홈 1장, 라이트·다크 짝 10장이
  캔버스 6번째 페이지 `v4 · 1단계 첫 실행`에 올라가 있다(캔버스 v19). 생성기는 `design/canvas/_tools/gen_v4.py`.
  사용자 검토 전이라 **커밋하지 않았다** — 승인되면 이 상태 그대로 커밋하고, 고칠 것이 있으면 `gen_v4.py`를 고쳐 다시 돌린다.
  `preview/*.png`는 이 맥에 크로미움이 없어 아직 못 찍었다. 다음은 2단계(내비 재편) 시안.
