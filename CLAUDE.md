...
- **홈 히어로 게이지 = 지금까지 쓴 돈 · 달력에서 적는다 (2026-09-24).** 사용자 피드백(게이지가 월말 예상이라 지금 쓴 돈이 안 보임 · 달력이 적는 곳으로 안 읽힘) → 전 · 후 그림 승인(게이지 https://claude.ai/artifact/1KhE4xZCngC3dyAv2pfUpM · 달력 https://claude.ai/artifact/X9bqAH616ZyB4pWHkDW33k) · 지시 「유기적으로 연결된 것 다 고려해서」. 히어로의 큰 숫자 · 게이지 · 설명 줄(`월급 360만원 중 112만원 썼어요`) · 오른쪽 줄(`목표까지 28.9%p 남음` · 기본 글자색) · 순자산 대비(1.2%) = 쓴 돈(`salaryRatio` · `burn`), 월말 예상(57.9%)은 배지(`월말에도 목표 안` · `월말엔 목표 초과` · `월말엔 월급 초과`)와 아래 칸(셋째 칸 `월말 예상 여유`/`월말 예상 초과`)으로만. 이력 부족도 같은 틀(배지 없음 · 아래 칸 — · 각주 두 줄 · `예상` → `월말 예상`). 달력 카드 제목 `이번 달 소비 기록` + 안내 줄 · 오늘 칸 테두리 1.5 + 0건이면 파란 `+` · 버튼 `+ 오늘 쓴 돈 적기` / 알약 `+ 더 적기`(줄 높이 40 · 저장 전후 높이 불변). 같이: 소비 탭 맨 위 · `이번 달 예상보다` · 또래 `내 월말 예상` · `순자산 대비 월말 예상` · 완료 카드 6순위 `월급의 31.1% → 31.4%` · 투어 홈 1 · 2 · 소개 · 홈 구성 고정 행. 샘플 9월 1일 12,000 → 1,008,000원(달력 9/1~8 합 = 히어로 112만원 · `LedgerV5` 9/1 12건 · `전체 24건`). 장 44(+ 다크 43) · 높이 7장 바뀜. 기록 `design/CHANGES-2026-09-24.md` · v5 계획 13판(§10 13판 메모) · v4 계획 §9 아래 메모.
  반영 뒤 독립 검증(72명 · 64건)과 재검토로 「최종 점검 후속」 1~20 · 「통합 점검」 21~27을 닫았다 — 이번 달 기록 0건이면 코치 · 다음 안내 · 또래 · 알림 · 자산 점수가 판정하지 않음(`이번 달 기록이 아직 없어요`) ·
  설명 줄은 움직이는 동안 보이는 값을 따르고 자리를 예약 · 월급 바로 아래는 원 단위 · 달력 안내는 알약 옆(샘플 문장만 위 줄) · 접힌 칸 수식어 줄 9(큰 글자 13) · 이체와 함께인 `+` 16 — 오늘 칸 배치는 시안과 앱이 0px 차.
