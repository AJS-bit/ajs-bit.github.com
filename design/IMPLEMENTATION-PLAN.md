...
| 항목 | 방법 | 결과 |
|---|---|---|
| 59개 아트보드 렌더 | Chromium 헤드리스로 각 프레임 크기에서 실제 렌더 | 전부 렌더됨 |
| 모바일 화면·모달 잘림 | 프레임 높이 대비 콘텐츠 높이를 DOM에서 측정 | 25개 모바일 아트보드 모두 **오버플로 0** — 하단 탭/저장 버튼이 잘리지 않음 |
...
| 캔버스 파일 무결성 | `seed-canvas.mjs --check` | ok, 60 files (59 아트보드 + canvas.json) |
...
- **다크 전용 아트보드가 없는 것** — 상태 카탈로그 4종(저장소·미입력·또래·확인)과 디자인 시스템 시트 2종은 라이트만 그렸습니다. 토큰 매핑으로 자동 대응하도록 설계했으나 렌더로 확인하지는 않았습니다. 데스크톱 소비 내역의 다크도 마찬가지입니다.
