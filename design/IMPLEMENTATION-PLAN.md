...
```
<RouteBar value={31.1} target={60} max={100} tone="positive" />   // 홈: value = 지금까지 쓴 돈 ÷ 월 실수령(2026-09-24 — 전에는 월말 예상 57.9)
```
...
| 파일 | 작업 | 대응 아트보드 |
|---|---|---|
| `app/page.tsx` | 홈 히어로 재구성(숫자 왼쪽 + 증감 오른쪽 + RouteBar + 3지표 + 근거줄/기준 조정 + 단일 CTA). 캡션 줄 오른쪽에 **순자산 대비 보조 지표**(`지금까지 쓴 돈 ÷ 순자산`, 13px · 2026-09-24 전에는 예상 소비 기준 — 히어로 값 정의는 `SPEC-COMPONENTS.md` §4 표) — 순자산 0이거나 자산 미입력이면 이 조각만 숨기고 주 지표는 그대로. 다음 안내 턴 카드. 보조 지표 3개 → `더 깊이 보기` 아코디언 3행. 데스크톱 1.6fr/1fr 그리드 | `Main`, `HomeScroll`, `DesktopHomeV5`(데스크톱 홈 — 옛 `DesktopHome`은 v3 원본 전용 기록), `EmptyStates` |
