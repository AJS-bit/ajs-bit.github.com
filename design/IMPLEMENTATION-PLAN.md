...
| `components/navi/coach-panel.tsx` | 조언 3개에 우선순위 번호 + `guidance` 스타일(왼쪽 3px 상태색). 절감 가정은 보라 점선 블록. 알림 패널은 아이콘+CTA 행 목록과 “조치 없음” 상태 | `CoachPanel`, `AlertsPanel` |
...
| 항목 | 방법 | 결과 |
|---|---|---|
| 67개 아트보드 렌더 | Chromium 헤드리스로 각 프레임 크기에서 실제 렌더 | 전부 렌더됨 |
| 전체 잘림 | 프레임 높이 대비 콘텐츠 높이를 DOM에서 측정 | **67개 모두 오버플로 0**. 상태 카탈로그 3종은 이 측정에서 잘림이 드러나 프레임 높이를 늘렸습니다(저장소 1180→1264, 또래 1360→1552, 미입력 1400→1718) |
...
| 캔버스 파일 무결성 | `seed-canvas.mjs --check` | ok, 68 files (67 아트보드 + canvas.json) |
...
- **아직 그리지 않은 화면** — 목적지 적립 모달, 목적지 5종 중 나머지 4종, 소비 과거 달 보기, 코칭의 "기록 없음" 상태. 토큰 시트는 라이트/다크 값을 한 장에 함께 적은 명세라 다크 사본을 두지 않습니다.
...
`.dc.html`은 정적 HTML + 인라인 스타일입니다. 브라우저에서 바로 열어도 되고, `navi-redesign.html` 하나만 열면 67개 아트보드를 한 캔버스에서 볼 수 있습니다.
