...
| 캔버스 파일 무결성 | `seed-canvas.mjs --check` | ok, 76 files (75 아트보드 + canvas.json) |
...
- **시안의 도착 예상일·완제일** — 화면에 적힌 월 배분·월 상환액으로 다시 계산하면 딱 떨어지지 않습니다. 여러 화면이 같은 날짜를 가리키도록 라벨을 맞춘 값이라, 검산 기준으로 쓰면 안 됩니다. 토큰 시트는 라이트/다크 값을 한 장에 함께 적은 명세라 다크 사본을 두지 않습니다.
