...
| `gen_v5.py` | **v5 · 1 · 2단계** 아트보드 17종과 다크 짝 — 홈 달력 5종(접힘 스트립 · 기본 5카드 전체 스크롤 `HomeDefaultScroll` 390 × 1330 · 펼침 월 달력 390/360 · 지난달 보기) + 하루 시트 쪽 8종(시트 기본 · 키보드를 내린 모습 `DaySheetScrolled` · 목록 우선 · 수정 · 360 × 640 · 완료 카드 · 분류하기 · 이력 부족 히어로) + 구현 참고 장 4종 = 저장을 한 번 더 물어보는 경우 1200 × 900 · 홈 맨 위 카드의 안내 줄 1200 × 640 · 달력 칸 읽는 법 1330 × 820 · 폭과 글자 크기 1150 × 1490 — 앱 화면이 아니라 설명 장이라 `spec_frame`으로 가로로 넓게 그린다. 펼침은 어느 폭에서도 7열 월 달력(계획 7판). 홈은 `Main.dc.html`을 잘라 쓰고 달력은 `gen_calendar`. **`gen_v4` · `gen_v4_stocks`를 import하므로 돌리면 v4 24종도 같이 다시 쓴다(멱등)** |
| `gen_v5_sheets.py` | **v5 · 하루 시트 · 완료 카드 · 상태 줄의 나머지 상태** 6종과 다크 짝 — 앱 화면 2종(소비 0건인 날 `DaySheetNoSpend` · 확인할 내용 목록 시트 `ReviewListSheet`) + 구현 참고 장 4종(`DaySheetNoSpendStates` 1200 × 1160 · `DoneCardStates` 1200 × 1090 · `DaySheetStates` 1210 × 1660 · `CalendarStatusLines` 1200 × 1050). `gen_v5`의 조각과 `w()`를 쓴다 — `import gen_v5`라 v4 · v5 장을 전부 다시 쓴다(멱등) |
| `gen_v5_screens.py` | **v5 · 달력 밖의 화면들** 6종과 다크 짝 — 구현 참고 장 5종(`InsufficientElsewhere` 1230 × 1330 · `FutureProvisional` 1230 × 1080 · `EtcSubline` 1200 × 970 · `LimitCardCases` 1200 × 960 · `ImportBackupNotes` 1230 × 1060) + 앱 화면 1종(`RecurringPrefill`). 손으로 쓴 `Spending` · `Limits` · `PeerStates` · `Future` · `Goals`는 `.dc.html`에서 잘라 쓰고, 모달 조각은 `gen_modals` · `gen_rest`를 import하지 않고 같은 모양으로 옮겨 적었다(import하면 그쪽 장을 다시 쓰므로). 계획에 원문이 없어 지은 문구 · 모양에는 `# 제안` 주석 |
