...
아트보드 67개를 일관된 조각으로 찍어내는 스크립트입니다. **최종 산출물은 상위 폴더의 `.dc.html` 파일**이고, 이 도구는 그것을 처음 만들 때와 다크 버전을 다시 뽑을 때 씁니다.
...
| `darken.py` | 라이트 아트보드 33종을 다크 토큰으로 변환. `<!--dc-keep-->…<!--/dc-keep-->` 구간은 변환하지 않는다(라이트에서도 반전된 토스트 등) |

```bash
python3 gen_screens.py && python3 gen_modals.py && python3 gen_errors.py 2030 && python3 darken.py && python3 gen_canvas.py
```
