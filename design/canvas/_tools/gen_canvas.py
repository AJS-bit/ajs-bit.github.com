...
NOTES = {
    ...
    'page-6': ('note-v4-stage1', 640,
               ...
               '구성 화면: 소비율 히어로와 이번 달 달력은 고정 행(둘 다 끌 수 없음 · 2026-09-22 사용자 결정), 아래 카드는 최대 4개. 행마다 왼쪽에 아이콘이 붙습니다(축소 미리보기는 서로 구별되지 않아 아이콘으로 바꿈). '
               '기본 5카드는 달력 · 다음 안내 · 이번 달 한도 · 대표 목적지 · 또래와 내 페이스이고, 홈 맨 아래 세 줄 카드(순자산 대비 소비 · 자산 종합 점수 · N년 뒤 순자산)는 「자산 한눈에」 한 행으로 목록에 남되 기본에서만 빠집니다(v5 계획 §3-1). 기본 배열은 v5 · 2단계와 함께 바뀌고 그 전까지 기본 홈은 v3 그대로입니다. '
               '이 시안은 다음 안내 · 이번 달 한도 · 순자산 한 줄 · 상환 계획 한 줄을 고른 상태라 4 / 4입니다. '
               '이 구성 화면은 처음 설치해 처음 실행할 때만 뜹니다. 그 뒤에는 설정 › 홈 구성으로만 들어갑니다(사용자 결정). 둘째 줄 둘째 · 셋째 장이 그 길입니다 — 설정 창의 「홈 구성 ›」 행과, 그 행으로 들어간 홈 구성 화면(건너뛰기 대신 닫기, 「이 구성으로 시작」 대신 저장 · 달력은 여기서도 고정 행이라 끌 수 없음). 둘째 줄 첫 장은 구성 화면에서 주식을 켠 변형 — 목록 끝에 주식 요약 행이 생기고 다음 안내 · 주식 요약 · 이번 달 한도 · 순자산 한 줄로 4 / 4입니다(달력은 고정 행)(3단계 페이지의 주식 켠 홈과 같은 구성). "주식도 볼까요?"는 기본 꺼짐 — '
    ...
    'page-8': ('note-v4-stage3', 640,
               ...
               '이 단계도 v5 뒤입니다 — 넷째 장의 홈은 달력 · 다음 안내 · 주식 요약 · 이번 달 한도 · 순자산(달력은 고정 행 · 4 / 4)이고 한도 라벨은 v5 · 3단계의 「앞으로 하루 47,270원」입니다.\n\n'
    ...
    'page-13': ('note-v5-elsewhere', 640,
               ...
               '둘째 줄(두 장 모두 구현 참고 장): 예상 기준을 확인하기 전 홈 밖의 화면들(소비 · 한도 · 코치 · 다음 안내 · 또래 · 알림에서 「순항 중」 같은 판단을 하지 않고 기록한 사실만 말함) · 미래 · 자산 경로의 「잠정」 표시.\n\n'
               '「잠정」 표시의 모양은 계획에 없어 시안에서 정했습니다(회색 작은 배지 · 보라 가정 카드와 구분).\n\n'
               '── 아래 줄은 다크입니다.'),
    'page-15': ('note-tour', 640,
               '첫 실행 안내 — 앱을 처음 설치해 첫 실행할 때, 탭에 처음 들어가면 그 탭의 안내가 1단계부터 뜹니다(2026-09-22 사용자 요청).\n\n'
               '홈 · 자산 · 소비 · 목적지 · 미래 각 3단계. 어두운 막 위에 요소 하나만 밝히고(브랜드 파랑 테두리) 그 아래 12px에 카드 — 요소가 화면 아래쪽이면 카드는 위. '
               '카드 = 「처음 안내」 알약 + 「탭 n / 3」 · 제목 16 / 700 · 본문 13 / 1.55 · 「건너뛰기」(그 탭의 남은 단계를 건너뜀) · 「다음」/「알겠어요」 40px.\n\n'
               '다시 보려면 설정 › 도움말 › 「처음 안내 다시 보기」. 저장은 settings.onboarding.tourSeen = {home, assets, spending, goals, future} — 보호 파일 · 백업 형식은 그대로. '
               '좌표는 gen_tour.py 의 표(실측)이고 탭 장을 고치면 다시 잽니다.\n\n'
               '── 아래 줄은 다크입니다.'),
