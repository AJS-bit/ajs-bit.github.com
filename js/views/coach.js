...
import { metrics, spendingLimit } from '../finance.js';
...
function whatIfTab() {
  ...
  return `
    <section class="card">
      ...
      <div class="field">
        <label>모든 카테고리를 <b style="color:var(--accent)">${whatif.cutPct}%</b> 줄인다면</label>
        <input class="range" type="range" min="0" max="40" step="1" value="${whatif.cutPct}" data-whatif="cutPct">
      </div>
      ${m.txCount === 0 ? empty('🧾', '이번 달 지출을 기록하면 절감 효과를 계산합니다.',
    ...
    <section class="card">
      ...
      <div class="field">
        <label>목표 월 소비율 <b class="num" style="color:var(--accent);float:right">${n(s.profile.targetBurn).toFixed(1)}%</b>
          (연 ${(n(s.profile.targetBurn) * 12).toFixed(1)}%)</label>
        <input class="range" type="range" min="0.2" max="10" step="0.1" value="${n(s.profile.targetBurn)}" data-act="target-burn">
      </div>
      ${m.net > 0 ? `
        <div class="kv"><span>목표 월 소비 상한</span><b class="num">${won((m.net * n(s.profile.targetBurn)) / 100)}</b></div>
        <div class="kv"><span>현재 예상 월 소비</span><b class="num ${m.projected > (m.net * n(s.profile.targetBurn)) / 100 ? 'neg' : 'pos'}">${won(m.projected)}</b></div>
        <div class="kv"><span>차이</span><b class="num ${m.projected > (m.net * n(s.profile.targetBurn)) / 100 ? 'neg' : 'pos'}">
          ${won(m.projected - (m.net * n(s.profile.targetBurn)) / 100)}</b></div>
        <div class="kv"><span>이 소비율을 유지하려면 필요한 순자산</span>
          <b class="num">${compact((m.projected * 100) / n(s.profile.targetBurn))}</b></div>
        <p class="hint" style="margin-top:10px">
          소비율은 <b style="color:var(--ink)">지출을 줄이거나</b> <b style="color:var(--ink)">자산을 늘려서</b> 낮출 수 있습니다.
          자산이 커질수록 같은 지출도 소비율이 자연히 내려갑니다.
        </p>` : `<p class="hint">순자산이 0보다 커야 목표 소비율을 계산할 수 있습니다.</p>`}
    </section>`;
}
