...
node test/legacy.mjs     # 구형 브라우저에서 빈 화면 대신 이유를 보여주는지
```

`legacy.mjs` 는 **오래된 엔진**을 본다. 이 앱의 `js/` 는 ES2020(옵셔널 체이닝·널 병합)을
190곳 남짓 쓰는데, 지원하지 않는 브라우저에서는 모듈이 파싱 단계에서 죽어 아무 코드도
실행되지 않는다. `type="module"` 자체는 Chrome 61부터 인식하므로 `nomodule` 대체도
걸리지 않아 **상단바만 남은 빈 화면**이 된다. APK 를 만들어 안드로이드 9 에뮬레이터
(WebView 66)에 올렸을 때 실제로 그랬다. `index.html` 의 고전 스크립트가 문법 지원을
직접 재서 이유를 알려주는지 확인한다.
